 ...
 *6.子组件的"props"发生变化时不会重新触发"componentDidMount","componentDidMount"只在组件第一次初始化时触发
 *7.componentWillReceiveProps()函数将要废弃,可通过getDerivedStateFromProps用来替换
 *8.props属性对于自身的组件来说是只读的,不能自己修改本身的props属性，只能通过外部修改组件的属性:
    1)通过父组件修改组件props值
    2)通过redux修改组件props值
 */
