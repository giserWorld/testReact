    ...
    render(){
        return(
            <div>
                ...
                {/* 1.路由跳转-params传值 */}
                <Link to="/router_param/name=xiaohong&id=2&code=51">1.1-link_router1(param)</Link><p/>
                <button onClick={()=>{this.clickFun('router_param')}}>1.2-btn_router1(param)</button><p/>

                {/* 2.路由跳转-search传值 */}
                <Link to="/router_search?type=search">2.1-link_router1(search)</Link><p/>
                <button onClick={()=>{this.clickFun('router_search')}}>2.2-btn_router1(search)</button><p/>

                {/* 3.路由跳转-query传值 */}
                <Link to={{pathname:'/router_query',query:{param1:'sunny',age:"25"}}}>3.1-link_router1(query)</Link><p/>
                <button onClick={()=>{this.clickFun('router_query')}}>3.2-btn_router1(query)</button><p/>

                {/* 4.路由跳转-state传值 */}
                <Link to={{pathname:'/router_state',state:{type:'state',age:"25"}}}>4.1-link_router1(state)</Link><p/>
                <button onClick={()=>{this.clickFun('router_state')}}>4.2-btn_router1(state)</button><p/>
